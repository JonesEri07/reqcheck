                    ...
                    <div className="space-y-2">
                      ...
                      <CodeBlock
                        code={(() => {
                          ...
                          return `${quizUrl}?${params.toString()}`;
                        })()}
...
const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || process.env.BASE_URL || "https://reqcheck.io";

const quizUrl = \`\${baseUrl}/quiz?companyId=\${companyId}&jobId=\${jobId}&redirectPass=\${encodeURIComponent(redirectPass)}&redirectFail=\${encodeURIComponent(redirectFail)}${hostedTestMode ? "&testMode=true" : ""}\`;
...
base_url = os.environ.get("BASE_URL", "https://reqcheck.io")
...
quiz_url = f"{base_url}/quiz?{urlencode(params)}"
...
$baseUrl = getenv('BASE_URL') ?: 'https://reqcheck.io';
...
$quizUrl = $baseUrl . '/quiz?' . http_build_query($params);
...
redirect_fail = "${hostedRedirectFail}"
${hostedTestMode ? "test_mode = true" : "# test_mode = false"}
base_url = ENV['BASE_URL'] || 'https://reqcheck.io'
...
quiz_url = "#{base_url}/quiz?#{URI.encode_www_form(params)}"
...
func generateQuizURL() string {
    ...
    quizURL := fmt.Sprintf("%s/quiz?%s", baseURL, params.Encode())
    return quizURL
}
...
export default function JobPage({ params }: { params: { id: string } }) {
  ...
  const quizUrl = new URL(\`\${baseUrl}/quiz\`);
