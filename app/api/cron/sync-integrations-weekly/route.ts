...
/**
 * POST /api/cron/sync-integrations-weekly
 * Weekly cron job to sync integrations with WEEKLY sync frequency
 *
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Run batch sync for weekly integrations
    const result = await batchSyncIntegrations(SyncFrequency.WEEKLY);
    ...
    console.error("Error in weekly integration sync cron job:", error);
    return NextResponse.json(
      {
        success: false,
...
/**
 * GET /api/cron/sync-integrations-weekly
